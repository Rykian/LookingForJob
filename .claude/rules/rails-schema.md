...
- **companies** (9 cols) - description:text, enriched_at:datetime, enrichment_version:integer, name:string, posts_as_final_client:boolean(=false), posts_as_recruiter:boolean(=false), website:string
- **company_aliases** (5 cols) - name:string, normalized_name:string | FK: company_id→companies | Idx: normalized_name(unique)
- **data_migrations** (0 cols)
- **job_offers** (36 cols) - canonical_id:bigint, city:string, company_name:string, content_fingerprint:string, description_html:text, disabled:boolean(=false), employment_type:string, final_client_guesses:jsonb(=[]), final_company_id:bigint, hybrid_remote_days_min_per_week:integer, keywords:string[](=[]), languages:jsonb(=[]), last_seen_at:datetime, location_mode:string, normalized_company_name:string, normalized_seniority:string, offer_language:string, posted_at:datetime, posted_by_recruiter:boolean, primary_technologies:string[](=[]), rejected:boolean(=false), salary_currency:string, salary_max_minor:integer, salary_min_minor:integer, score:integer, score_breakdown:jsonb, secondary_technologies:string[](=[]), source:string, steps_details:jsonb(={}), title:string, url:string, url_hash:string | FK: commute_city_id→commute_cities, company_id→companies, company_id→companies, job_offer_id→job_offers | Idx: url(unique), url_hash(unique)
