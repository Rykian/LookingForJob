...
2. Discovery validated on real URL, including logged-in vs logged-out comparison when possible.
3. Analyze validated on real URL and user approved result.
4. Missing fields are clearly listed and mapped.
5. `description_html` is minimized for token efficiency.
6. Enrich infers missing fields from stripped description text.
7. Provider-specific specs exist for integration behavior.
