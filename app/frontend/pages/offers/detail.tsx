...
type OfferDetailData = {
  jobOffer: {
    ...
    normalizedSeniority: string | null
    offerLanguage: string | null
    englishLevelRequired: string | null
    score: number | null
    scoreBreakdown: Record<string, unknown> | null
    primaryTechnologies: string[] | null
    secondaryTechnologies: string[] | null
...
export default function OfferDetailPage() {
  const { id } = useParams()
  const { data, loading, error } = useQuery<OfferDetailData>(OFFER_QUERY, {
    variables: { id },
    skip: !id,
  })
  ...
  return (
    <div className="space-y-6 p-8">
      <div>
        <h1 className="text-2xl font-semibold">{offer.title || `Offer #${offer.id}`}</h1>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          ...
          <p>
            ...
            {new Date(offer.firstSeenAt).toLocaleString()} → {new Date(offer.lastSeenAt).toLocaleString()}
          </p>
          <a className="text-primary underline-offset-4 hover:underline" href={offer.url} target="_blank" rel="noreferrer">
            Open source listing
          </a>
        </CardContent>
      </Card>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(offer.primaryTechnologies || []).map((tech) => (
                  <Badge key={`p-${tech}`} variant="secondary">{tech}</Badge>
                ))}
                {(offer.primaryTechnologies || []).length === 0 ? <span className="text-muted-foreground">-</span> : null}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(offer.secondaryTechnologies || []).map((tech) => (
                  <Badge key={`s-${tech}`} variant="outline">{tech}</Badge>
                ))}
                {(offer.secondaryTechnologies || []).length === 0 ? <span className="text-muted-foreground">-</span> : null}
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div
            ...
            dangerouslySetInnerHTML={{ __html: offer.descriptionHtml || '<p>No description available.</p>' }}
          />
        </CardContent>
      </Card>
    </div>
